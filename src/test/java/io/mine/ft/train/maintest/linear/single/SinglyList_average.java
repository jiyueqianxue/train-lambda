...
/*
 * 程序运行结果如下： list=(10,20,30) average(list)=20.0 averageAll(list)=20.0
 * averageExceptMaxMin(list)=20.0
 * 
 */
/*
 * 修改 double average(SeqList<Student> list) //求平均值 { int n=stulist.length();
 * //顺序表长度 double sum=0; for (int i=0; i<n; i++) sum += stulist.get(i).score;
 * //此处get(i)返回Student结构类型 return n>0 ? sum/n: 0; //给出0个元素结果，避免除数为0错误 } double
 * average(SinglyList<Student> list) //求平均值 { int n=stulist.length(); //单链表长度 if
 * (n>0) { double sum=0; for (int i=0; i<n; i++) sum += stulist.get(i).score;
 * //此处get(i)返回Student结构类型 return sum/n; } return 0.0; //给出0个元素的结果，避免除数为0错误 }
 */
